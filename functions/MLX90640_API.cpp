...
//@BD, Tell if a frame is ready
int MLX90640_FrameReady(uint8_t slaveAddr)
{
    ...
    return statusRegister & (0x0001|0x0008);

}

int MLX90640_ClearFrameAvailFlag(uint8_t slaveAddr)
{
...
//@BD, Start a measurement in step mode
int MLX90640_StartMeasurement(uint8_t slaveAddr)
{
    uint16_t statusRegister;
    int error;
    ...
    if(error == 0)
    {
        //We set the start bit but we also clear the data bit
        error = MLX90640_I2CWrite(slaveAddr, 0x8000, (statusRegister | 0x0010) & 0xFFF7); 
    }    
...
//@BD: Set the step mode bit.
int MLX90640_SetStepMode(uint8_t slaveAddr, int step_mode)
{
    ...
    {
        if(step_mode)
            error = MLX90640_I2CWrite(slaveAddr, 0x800D, controlRegister1 | 0x0004);        
        else
            error = MLX90640_I2CWrite(slaveAddr, 0x800D, controlRegister1 & 0xFFFB);
    }    
