 ...
 *
 * @copyright (C) Brice Dubost (code indicated by @BD)
 *
...
//@BD Code for other kind of I2C writing for the other sensors/LCD/Servo
int I2C_byte_write(uint8_t slaveAddr, uint8_t data)
{
    if(!i2c_fd){
        i2c_fd = open(i2c_device, O_RDWR);
    }
    char cmd[1] = {(char)(data & 0x00FF)};
    int result;
...
int I2C_byte_write_data(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{ 
    char cmd[2] = { (char)(writeAddress & 0x00FF), (char)(data & 0x00FF)};
    int result;

    //printf("I2C Will write cmd 0x%02x 0x%02x to address 0x%02x\n", cmd[0], cmd[1], slaveAddr);
    ...
    if (ioctl(i2c_fd, I2C_RDWR, &i2c_messageset) < 0) {
        printf("I2C Write Error! errno %d\n", errno);
        printf("I2C tried to write cmd 0x%02x 0x%02x to address 0x%02x\n", cmd[0], cmd[1], slaveAddr);
...
int I2C_word_write_data(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
    char cmd[3] = { (char)(writeAddress & 0x00FF), (char)(data & 0x00FF), (char)(data >> 8)};
    int result;
    //printf("I2C Will write cmd 0x%02x 0x%02x 0x%02x to address 0x%02x\n", cmd[0], cmd[1], cmd[2], slaveAddr);
    ...
    if (ioctl(i2c_fd, I2C_RDWR, &i2c_messageset) < 0) {
        printf("I2C Write Error! errno %d\n", errno);
        printf("I2C tried to write cmd 0x%02x 0x%02x 0x%02x to address 0x%02x\n", cmd[0], cmd[1], cmd[2], slaveAddr);
        return -1;
    }
